...
#pragma mark - 获取相册 iOS8+
- (void)getPreiOS8LastImageWithImageView:(UIImageView *)imageView {
    ...
    [al latestAsset:^(ALAsset * _Nullable asset, NSError * _Nullable error) {
        ...
        if ([type isEqual:ALAssetTypePhoto]){
            ...
            if (needImage) {
                imageView.image = needImage;
                [self showSuitFrame:imageView img:needImage];
            }
        }
    }];
}
...
- (void)latestAsset:(void (^)(ALAsset * _Nullable, NSError *_Nullable))block {
    [self enumerateGroupsWithTypes:ALAssetsGroupSavedPhotos usingBlock:^(ALAssetsGroup *group, BOOL *stop) {
        if (group) {
            ...
            [group enumerateAssetsWithOptions:NSEnumerationReverse/*遍历方式*/
